    ...
    public function TratarErro($pdoE, $obj, $comando) {
        ...
        switch ($pdoE->getCode()) {
            ...
            case 1054: // CAMPO NAO EXISTE        
                ...
                if (empty($configuracaoCampoInexistente['relacionamento']) && in_array($campoInexistente, $atributosClasse))
                    ...
                    $classeEstrangeira = strval($_Biscoito->getClasseRelacionamento($this->tabela, $campoInexistente));
                    $arrClasseEstrangeira = explode('\\', $classeEstrangeira);
                    $tabelaEstrangeira = array_pop($arrClasseEstrangeira);
                    ...
                    if ($configuracaoCampoInexistente['cardinalidade'] == '1') {
                        ...
                        if (!empty($tabelaEstrangeira))
                            $orm->AdicionarChaveEstrangeira($nomeChaveEstrangeira, $this->tabela, $campoInexistente, $tabelaEstrangeira, 'id', new $classeEstrangeira);
                    }
                    else if ($configuracaoCampoInexistente['cardinalidade'] == 'N') {
                        $nulo = ($configuracaoCampoInexistente['nulo'] == 'true') ? '' : 'NOT NULL';
                        $orm->AdicionarCampo($this->tabela, $campoInexistente, "INTEGER $nulo", $this->obj);
