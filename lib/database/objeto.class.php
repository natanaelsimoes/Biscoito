...
class TObjeto {
    ...
    public static function QuantidadeRegistrados($whereCampo = null, $whereRelacao = null, $whereValor = null) {
        $namespace = get_called_class();
        $table = TDatabaseUtil::getClasseNamespace($namespace);
        $query = "SELECT count(id) FiQuantidade FROM $table";
        if (!is_null($whereValor))
            $query.= " WHERE $whereCampo $whereRelacao $whereValor";
        $bd = new TDatabase;
        $bd->AbrirConexao();
        $quantidade = $bd->Selecionar($query, new $namespace);
        return $quantidade[0]->FiQuantidade;
    }

    public static function ListarTodos($pagina = 1, $quantidade = null) {
        $namespace = get_called_class();
        $table = TDatabaseUtil::getClasseNamespace(get_called_class());
        return $namespace::ListarTodosOrdenadoPor("$table.id DESC", $pagina, $quantidade);
    }

    public static function ListarTodosOnde($filtroOnde, $pagina = 1, $quantidade = null) {
        $namespace = get_called_class();
        $query = sprintf('%s %s %s', $namespace::MontarSelectBasico(), 'WHERE', $filtroOnde);
        $bd = new TDatabase;
        $bd->AbrirConexao();
        return $bd->Selecionar($query, new $namespace, $pagina, $quantidade);
    }

    public static function ListarPorId($id) {
        $namespace = get_called_class();
        $table = TDatabaseUtil::getClasseNamespace($namespace);
        $dados = $namespace::ListarTodosOnde("$table.id = $id");
        return $dados[0];
    }

    public static function ListarTodosOrdenadoPor($filtroOrdem, $pagina = 1, $quantidade = null) {
        $namespace = get_called_class();
        $query = sprintf('%s %s', $namespace::MontarSelectBasico(), " ORDER BY $filtroOrdem ");
        $bd = new TDatabase;
        $bd->AbrirConexao();
        return $bd->Selecionar($query, new $namespace, $pagina, $quantidade);
    }

    public static function ListarTodosOndeOrdenadorPor($filtroOnde, $filtroOrdem, $pagina = 1, $quantidade = null) {
        $namespace = get_called_class();
        $query = sprintf('%s %s', $namespace::MontarSelectBasico(), " WHERE $filtroOnde ORDER BY $filtroOrdem ");
        $bd = new TDatabase;
        $bd->AbrirConexao();
        return $bd->Selecionar($query, new $namespace, $pagina, $quantidade);
    }

    private static function MontarSelectBasico() {
        global $_Biscoito;
        $namespace = get_called_class();
        $fields = $joins = "";
        $tableEquals = 0;
        $table = TDatabaseUtil::getClasseNamespace($namespace);
        $xmlConfig = $_Biscoito->getConfiguracaoXML($_Biscoito->getModulo($namespace));
        foreach (array_keys(TObjeto::MapearClasse(new $namespace)) as $col) {
            $colXml = str_replace("$table.", '', $col);
            if (strval($xmlConfig->classes->$table->{$colXml}['cardinalidade']) != 'N')
                $fields.= "$col,";
        }
        $fields = substr($fields, 0, -1);
        foreach ($xmlConfig->classes->$table as $class)
            foreach (get_object_vars($class) as $attr => $value) {
                $cardinalidade = strval($class->{$attr}['cardinalidade']);
                $foreignNamespace = explode('\\', $class->{$attr}['relacionamento']);
                $foreignTable = $foreignTableAlias = array_pop($foreignNamespace);
                if (!empty($foreignTable) && $cardinalidade == '1') {
        ...
        return $query = " SELECT $table.id FiId,$fields FROM $table $joins ";
    }

    private static function MontarSelectComplementar($tabela, $namespaceEstrangeiro) {
        global $_Biscoito;
        $xmlConfig = $_Biscoito->getConfiguracaoXML($_Biscoito->getModulo($namespaceEstrangeiro));
        $arrEstrangeiro = explode('\\', $namespaceEstrangeiro);
        $tabelaEstrangeira = array_pop($arrEstrangeiro);                
        foreach (array_keys(TObjeto::MapearClasse(new $namespaceEstrangeiro)) as $col) {
            $colXml = str_replace("$tabelaEstrangeira.", '', $col);
            if (strval($xmlConfig->classes->$tabelaEstrangeira->{$colXml}['cardinalidade']) != 'N')
                $fields.= "$col,";
        }
        $fields = substr($fields, 0, -1);
        $tabelaHibrida = sprintf('%s_%s', $tabela, $tabelaEstrangeira);
        $joins = " JOIN $tabelaHibrida $tabelaHibrida ON $tabelaHibrida.{$tabelaEstrangeira}_id = {$tabelaEstrangeira}_id ";
        $joins .= " AND $tabelaHibrida.{$tabela}_id = {$this->id()} ";
        return $query = " SELECT $tabelaEstrangeira.id FiId,$fields FROM $tabelaEstrangeira $joins ";
    }

    public static function MapearClasse($obj) {
            ...
            if (preg_match('/^\s*\[\s*(\S+)\s*\]\s+=>\s+(.*)$/', $line, $matches)) {
                ...
                if (strpos($matches[0], 'TObjeto') === false && mb_stripos($matches[2], 'array') === false) {
                    ...
                    if (mb_stripos($matches[2], 'array') !== false) {
                        ...
                        else if ($ARR_LIST_CLASS == $obj_class)
                            $ret_list[$matches[1]] = $matches[2];
                    }
                }
            } else if (mb_stripos($line, ')') !== false) {
                if ($arr_index >= 0) {
